...
var servicesUtils = require('~/cartridge/scripts/services/servicesUtils')
...
function handle(currentBasket, paymentInformation, paymentMethodID, req) {    
    ...
    Transaction.wrap(function() {
        ...
        paymentInstrument.setCreditCardToken(paymentInformation.cardToken);
        
        // set custom attributes
        paymentInstrument.custom.boltCardLastDigits = paymentInformation.cardLastFourDigits;
        paymentInstrument.custom.boltCardBin = paymentInformation.cardBin;
...
function authorize(orderNumber, paymentInstrument, paymentProcessor) {
    ...
    // send auth call
    var response = boltHttpUtils.restAPIClient(constants.HTTP_METHOD_POST, constants.AUTH_CARD_URL, JSON.stringify(authRequestObj.authRequest));
    if (response.status && response.status === HttpResult.ERROR) {
        log.error(
          'Payment authorization failed, error: ' +
            (!empty(response.errors) && !empty(response.errors[0].message) ? response.errors[0].message : '')
        );
       return { error: true };
    }
    ...
    Transaction.wrap(function () {
        var transactionRef = response.transaction && response.transaction.reference ? response.transaction.reference : orderNumber;
...
function getAuthRequest(order, paymentInstrument) {
    ...
    var creditCardInfo = {
        ...
        number: '',
        expiration: StringUtils.formatNumber(paymentInstrument.getCreditCardExpirationYear(), '0000') + '-' + StringUtils.formatNumber(paymentInstrument.getCreditCardExpirationMonth(), '00'),
    ...
    var request = {
        ...
        division_id: Site.getCurrent().getCustomPreferenceValue('boltMerchantDivisionID') || '',
